...
Microsoft Presidio 提供 PII 检测和匿名化，用于：
- 响应清理（在交付给用户之前）
- LLM 提示词过滤（在发送数据给 LLM 之前）
- 审计日志脱敏

### 8.2 集成

框架根据领域模型 PII 规则自动配置 Presidio。配置是声明式的：
...
pii:
  ...
  # Analyzer 和 Anonymizer 引擎从此配置自动初始化。
  # 用途：响应清理、LLM 提示词过滤、审计日志脱敏。

# domain-models/home-insurance.yaml（要检测的 PII 实体）
pii_rules:
  entities:
    - PHONE_NUMBER
    - EMAIL_ADDRESS
    - PERSON
    - CN_ID_NUMBER           # 自定义：中国身份证号
    - CREDIT_CARD
  masking_strategy: partial_mask
```
