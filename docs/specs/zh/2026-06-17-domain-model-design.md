...
守卫是针对当前实体状态求值的布尔表达式。框架为守卫提供最小的表达式语言。
...
| 变量 | 求值为 |
|----------|-------------|
| `context_incomplete` | 若任一必填字段为 null/空则为 `true` |
| `context_complete` | 若所有必填字段均为非空且非空字符串则为 `true` |
...
守卫表达式语法被刻意设计为最小化。对于复杂的业务规则（例如 "risk_score < 80 AND coverage_amount > 500000"），框架委托给：
- **自定义守卫函数** — 以函数名引用的已注册 Python 可调用对象
- **规则引擎守卫** — 当 `validate_strategy` 使用规则引擎时，守卫可被编译为引擎的原生格式
